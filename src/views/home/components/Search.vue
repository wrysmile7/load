<template>
  <div class="search-box">
    ...
    <div class="right">
      ...
      <el-button class="delbtns" @click="deleteList('select')"
        >批量删除</el-button
      >
      <div class="date-box" v-if="tablename !== 'device'">
        <span>按日期删除：</span>
    ...
    <!-- 新增设备弹框 -->
    <AddDevices :addVisible.sync="addVisible"></AddDevices>
  </div>
</template>
<script>
...
export default {
  ...
  watch: {
    ...
    options: {
      handler(val) {
        this.optionList = JSON.parse(JSON.stringify(val)).filter(
          v => v !== 'ID'
        )
      },
      deep: true,
